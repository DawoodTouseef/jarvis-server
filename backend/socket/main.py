...
from backend.models.users import Users, UserNameResponse
from backend.models.channels import Channels
from backend.models.chats import Chats
from backend.models.notes import Notes, NoteUpdateForm
from backend.utils.redis import (
...
from backend.socket.utils import RedisDict, RedisLock, YdocManager
from backend.tasks import create_task, stop_item_tasks
from backend.utils.redis import get_redis_connection
from backend.utils.access_control import has_access, get_users_with_access
...
if CORS_ALLOW_ORIGIN:
    ...
    if isinstance(CORS_ALLOW_ORIGIN, str):
        # Use the same separator as the main application (semicolon)
        cors_origins = [origin.strip() for origin in CORS_ALLOW_ORIGIN.split(';')]
    elif isinstance(CORS_ALLOW_ORIGIN, list):
...
@sio.on("get_location")
async def get_location(sid, data):
    log.info(f"get_location: {data}")
    ...
    try:
        ...
        # Create entity ID based on user name
        entity_id = f"person.{data['name']}"
        ...
        if data.get("accuracy") is not None:
            attributes["accuracy"] = data.get("accuracy")
        if data.get("altitude") is not None:
            attributes["altitude"] = data.get("altitude")
        if data.get("speed") is not None:
            attributes["speed"] = data.get("speed")
        
        # Create or update the state
        state = StatesCtrl.create_state(
            entity_id=entity_id,
            state="person",  # Default to "person" state
        ...
        if state:
            ...
            log.info(f"Location state updated for {entity_id}")
        else:
