...
from backend.models.homeassistant import (
    Events)
from backend.models.users import Users, UpdateProfileForm
from backend.models.groups import Groups
...
from backend.config import OPENID_PROVIDER_URL, ENABLE_OAUTH_SIGNUP, ENABLE_LDAP
...
from backend.utils.access_control import get_permissions
...
async def extend_signup(request: Request, response: Response, form_data: ExtendSignupForm):
    ...
    if form_data.location and any([
        form_data.location.get("latitude"),
        form_data.location.get("longitude"),
        form_data.location.get("address")
    ]):
        try:
            # Import the States controller
            ...
            # Create a state with the location data
            location_state = StatesCtrl.create_state(
                entity_id=entity_id,
