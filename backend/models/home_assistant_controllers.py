...
from backend.models.homeassistant import (
    ...
    RecorderRuns, SchemaChanges, MigrationChanges, StatisticsRuns
)
...
class EventsController:
    """Controller for Events table operations."""
...
####################
# States Table Controller
####################

class StatesController:
    """Controller for States table operations."""
...
####################
# Statistics Table Controller
####################

class StatisticsController:
    """Controller for Statistics table operations."""
...
class StatesMetaController:
    """Controller for StatesMeta table operations."""
...
class EventDataController:
    """Controller for EventData table operations."""
    
    def create_event_data(self, data: Dict[str, Any]) -> Optional[EventData]:
        ...
        try:
            ...
            log.error(f"Error creating event data: {e}")
            return None
...
class EventTypesController:
    """Controller for EventTypes table operations."""
    
    def create_event_type(self, event_type: str) -> Optional[EventTypes]:
        """Create new event type."""
        try:
            with get_db() as db:
                event_type_obj = EventTypes(event_type=event_type)
                db.add(event_type_obj)
                db.commit()
                db.refresh(event_type_obj)
                return event_type_obj
        except Exception as e:
            log.error(f"Error creating event type: {e}")
            return None


####################
# State Attributes Controller
####################

class StateAttributesController:
    """Controller for StateAttributes table operations."""
    
    def create_state_attributes(self, attributes: Dict[str, Any]) -> Optional[StateAttributes]:
        """Create new state attributes."""
        try:
            with get_db() as db:
                attrs_obj = StateAttributes(
                    hash=hash(str(attributes)),
                ...
                db.add(attrs_obj)
                db.commit()
                db.refresh(attrs_obj)
                return attrs_obj
        except Exception as e:
            log.error(f"Error creating state attributes: {e}")
            return None


####################
# Statistics Meta Controller
####################

class StatisticsMetaController:
    """Controller for StatisticsMeta table operations."""
    
    def create_statistics_meta(
        self, 
        statistic_id: str, 
        source: str = "recorder",
    ...
    ) -> Optional[StatisticsMeta]:
        """Create new statistics metadata."""
        try:
            with get_db() as db:
                meta = StatisticsMeta(
                    statistic_id=statistic_id,
            ...
            log.error(f"Error creating statistics metadata: {e}")
            return None


####################
# Recorder Runs Controller
####################

class RecorderRunsController:
    """Controller for RecorderRuns table operations."""
    
    def create_recorder_run(
        self,
        start: datetime,
        end: Optional[datetime] = None,
        closed_incorrect: bool = False
    ) -> Optional[RecorderRuns]:
        """Create new recorder run."""
        try:
            with get_db() as db:
                run = RecorderRuns(
                    start=start,
                    end=end,
                    closed_incorrect=closed_incorrect,
                    created=datetime.now()
                )
                db.add(run)
                db.commit()
            ...
            log.error(f"Error creating recorder run: {e}")
            return None
...
class SchemaChangesController:
    """Controller for SchemaChanges table operations."""
    
    def create_schema_change(self, schema_version: int) -> Optional[SchemaChanges]:
        """Create new schema change record."""
        try:
            with get_db() as db:
                change = SchemaChanges(
            ...
            log.error(f"Error creating schema change: {e}")
            return None
...
class MigrationChangesController:
    """Controller for MigrationChanges table operations."""
    
    def create_migration_change(self, migration_id: str, version: int) -> Optional[MigrationChanges]:
        """Create new migration change record."""
        try:
            with get_db() as db:
                change = MigrationChanges(
            ...
            log.error(f"Error creating migration change: {e}")
            return None


####################
# Statistics Short Term Controller
####################

class StatisticsShortTermController:
    """Controller for StatisticsShortTerm table operations."""
    
    def create_short_term_statistic(
        self,
        statistic_id: str,
        start: datetime,
        mean: Optional[float] = None,
        ...
        """Create a new short-term statistic."""
        try:
            with get_db() as db:
                # Ensure statistics metadata exists
                meta = db.query(StatisticsMeta).filter(StatisticsMeta.statistic_id == statistic_id).first()
            ...
            log.error(f"Error creating short-term statistic: {e}")
            return None


####################
# Statistics Runs Controller
####################

class StatisticsRunsController:
    """Controller for StatisticsRuns table operations."""
    
    def create_statistics_run(self, start: datetime) -> Optional[StatisticsRuns]:
        """Create new statistics run."""
        try:
            with get_db() as db:
                run = StatisticsRuns(start=start)
                db.add(run)
                db.commit()
                db.refresh(run)
                return run
        except Exception as e:
            log.error(f"Error creating statistics run: {e}")
            return None


# Create singleton instances for each controller
EventsCtrl = EventsController()
StatesCtrl = StatesController()
StatisticsCtrl = StatisticsController()
StatesMetaCtrl = StatesMetaController()
EventDataCtrl = EventDataController()
EventTypesCtrl = EventTypesController()
StateAttributesCtrl = StateAttributesController()
StatisticsMetaCtrl = StatisticsMetaController()
RecorderRunsCtrl = RecorderRunsController()
SchemaChangesCtrl = SchemaChangesController()
MigrationChangesCtrl = MigrationChangesController()
StatisticsShortTermCtrl = StatisticsShortTermController()
StatisticsRunsCtrl = StatisticsRunsController()