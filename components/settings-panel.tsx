...
import {
  User, Bell, Shield, Save, Upload, Settings, Lock, Copy, EyeOff, Eye, AudioLines, Download, Home, Plus, Edit, Trash2, Building, MapPin, Layers, Heart, Camera,
  Earth, PlugIcon,
  Cloud
} from "lucide-react"
...
export function SettingsPanel() {
  ...
  const delete_api_key = () => {
    apiClient.delete_api_key().then((response) => {
      if (response.success && response.data) {
        setapikey("");
      }
    }).catch((error) => {
  ...
  const get_api_key = () => {
    apiClient.get_api_key().then((response) => {
      if (response.success && response.data) {
        setapikey(response.data.api_key);
      }
    }).catch((error) => {
